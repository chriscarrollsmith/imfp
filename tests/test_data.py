...
def test_imf_databases():
    ...
    assert result.isna().sum().sum() == 0, (
            'Result should not contain any NAs'
        )
...
def test_imf_dataset_error_handling():
    ...
    with pytest.raises(Exception):
        imf_dataset(
                database_id="APDREO201904", counterpart_area="X",
                counterpart_sector="X", times=1
            )
    with pytest.warns(Warning):
        imf_dataset(
                database_id="APDREO201904", ref_area="AU",
                indicator=["BCA_BP6_USD", "XYZ"]
            )
    with pytest.raises(Exception):
        imf_dataset(times=1)
    with pytest.raises(Exception):
    ...
    with pytest.raises(Exception):
        imf_dataset(
                database_id="WHDREO201910", freq="M", ref_area="US",
                indicator=["PPPSH", "NGDPD"], start_year=2010, end_year=2011
            )
    with pytest.warns(Warning):
        imf_dataset(
                database_id="FISCALDECENTRALIZATION",
                parameters=params, ref_sector=["1C_CG", "1C_LG"]
            )
...
def test_imf_dataset_vector_parameters_request():
    df = imf_dataset(
            database_id="AFRREO", indicator=["TTT_IX", "GGX_G01_GDP_PT"],
            ref_area="7A", start_year=2021
        )
    assert len(df) > 1
    assert all(int(date) >= 2021 for date in df['time_period'])
    assert all(
            indicator in ["TTT_IX", "GGX_G01_GDP_PT"]
...
def test_imf_dataset_data_frame_prep():
    case_1 = imf_dataset(
            database_id="WHDREO201910", freq="A", ref_area="US",
            indicator=["PPPSH", "NGDPD"], start_year=2010, end_year=2012
        )
    case_2 = imf_dataset(
            database_id="WHDREO201910", freq="A", ref_area="US",
            indicator=["PPPSH", "NGDPD"], start_year=2010, end_year=2011
    ...
    case_3 = imf_dataset(
            database_id="WHDREO201910", freq="A", ref_area="US",
            indicator=["NGDPD"], start_year=2011, end_year=2012
        )

    desired_names = [
            "time_period", "obs_value", "freq", "ref_area",
            "indicator", "unit_mult", "time_format"
        ]

    assert len(case_1) == 4 and len(case_2) == 2 and len(case_3) == 2
    assert (
                len(case_1.columns) == 7 and
                len(case_2.columns) == 7 and
                len(case_3.columns) == 7
            )
    assert (
                all(col in desired_names for col in case_1.columns) and
...
def test_imf_dataset_include_metadata():
    output = imf_dataset(
            database_id="WHDREO201910", freq="A", ref_area="US",
            indicator=["PPPSH", "NGDPD"], start_year=2010,
            end_year=2012, include_metadata=True
        )
